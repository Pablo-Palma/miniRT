...
/*   Updated: 2024/04/17 12:38:06 by mamagalh@st      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int shadow_plane(t_scene *scene, t_vec3 hit_point)
{
    ...
    if (intersect_ray_cyl(origin_offset, light_direction, scene->cyl, &t)) {
        if (t < light_distance) {
            return 1; // El punto está en sombra
        }
    }
...
int	shadow(t_scene *scene, t_vec3 hit_point, t_light light, t_vec3 normal)
{
	...
	t_vec3	origin_offset = vector_add(hit_point, vector_scale(normal, 0.001)); //Ajustar punto de origen y evitar auto-intersección.
	double	t = INFINITY;

	if (intersect_ray_sphere(origin_offset, light_dir, scene->sphere, &t) && t < distance)
			return (1);
	if (intersect_ray_cyl(origin_offset, light_dir, scene->cyl, &t) && t < distance)
			return (1);
	return (0);
}
