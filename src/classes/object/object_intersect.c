...
static int	intersect_cyl(t_cyl cyl, t_ray *ray)
{
	...
	// 	t_vec3 diff_vector = vector_sub(hit_point, cyl.center);
	// 	double dot_product = vector_dot_product(diff_vector, cyl.dir);
	// 	cap_plane = (t_plane){.point = diff_vector, .normal = cyl.dir, .color = cyl.color};
	// 	if (dot_product > 0)
	// 		*ray->obj = (t_plane){.point = diff_vector, .normal = cyl.dir, .color = cyl.color};
	// 	else
	// 		*ray->obj = (t_plane){.point = diff_vector, .normal = vector_negate(cyl.dir), .color = cyl.color};
	// }
	return (0);
}
		// t_vec3	hit_point = vector_add(ray_origin, vector_scale(ray_dir, t));
